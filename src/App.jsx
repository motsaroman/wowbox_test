...
export default function App() {
  ...
  const getRecommendedBox = () => {
    // 2-й индекс = 3-й вопрос: "Что больше всего нравится?"
    const styleAnswer = quizAnswers[2];

    const styleMap = {
      practical: "techno",  // Гаджеты и техно
      emotions: "cozy",     // Уют и комфорт
      quality: "party",     // Веселье и игры
      surprise: "sweet",    // Сладости и вкусняшки
    };
  ...
  const handleQuizOrder = () => {
    // 3-й вопрос (индекс 3) – "Что важнее всего в подарке?"
    const importanceAnswerId = quizAnswers[3];
    const importanceQuestion = quizData[3];

    const importanceOption = importanceQuestion.options.find(
      (o) => o.id === importanceAnswerId
    );

    const importanceText = importanceOption
      ? `Что важнее в подарке: ${importanceOption.title}`
      : "";

    // прокидываем это как комментарий в персонализацию
    setBoxPersonalization((prev) => ({
      ...(prev || {}),
      additionalWishes: importanceText,
    }));

    // из квиза начинаем с шага 3 — "Есть ли ограничения?"
    setPersonalizationStartStep(3);
        ...
        <>
          ...
          <main>
            <div className={styles.selectYourOwnWowbox}>
              ...
              <BoxesCarousel
                onOrderClick={() => {
                  setPersonalizationStartStep(1);      // обычный заказ – с первого шага
                  setIsPersonalizationOpen(true);
                }}
              />
          ...
          <BoxingPersonalization
            ...
            startStep={personalizationStartStep}
