...
async function createOrderInRetailCRM(data) {
  if (!process.env.RETAILCRM_URL || !process.env.RETAILCRM_API_KEY) {
    console.warn("RetailCRM credentials missing (URL or API Key not found)");
    return null;
  }
  ...
  try {
    ...
    if (data.delivery.type === '5post') {
      ...
      deliveryAddressText = data.delivery.fullAddress; 
    }

    const combinedComment = [
      data.comments.user,
      data.comments.courier ? `Комментарий курьеру: ${data.comments.courier}` : null
    ].filter(Boolean).join('\n');
    const crmOrder = {
      orderMethod: 'site',
      firstName: data.contact.name,
      ...
      items: [
        {
          ...
          offer: {
            displayName: data.boxTitle,
            name: data.boxTitle,
            // externalId: '...' // Если нужно привязать к складу
          }
        },
        {
          productName: "Доставка",
    ...
    if (response.data.success) {
      ...
      console.error('RetailCRM Error Response:', response.data);
      return null;
    }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let baseDeliveryPrice = DELIVERY_RATES[cityFias] || DEFAULT_DELIVERY_PRICE;

    let deliveryPrice = baseDeliveryPrice;
